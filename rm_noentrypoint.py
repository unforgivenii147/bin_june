...
def remove_wheels_without_entry_points(
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        future_to_wheel = {executor.submit(check_wheel_for_entry_points, wheel): wheel for wheel in wheel_files}
        wheels_to_remove = []
        wheels_to_keep = []
...
def main():
    ...
    parser.add_argument(
        "-w", "--workers", type=int, default=None, help="Maximum number of parallel workers (default: CPU count)"
    )
    parser.add_argument(
