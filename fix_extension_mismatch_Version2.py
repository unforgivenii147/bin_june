...
# MIME type -> file extension mapping (used by python-magic detection)
MIME_TO_EXT = {
    # Images
    "image/png": ".png",
    "image/jpeg": ".jpg",
...
def detect_with_magic(path: Path) -> Optional[Tuple[str, str]]:
    """Use python-magic to detect file type via MIME type.
...
def detect_file_type(path: Path) -> Optional[Tuple[str, str]]:
    """Detect file type using python-magic if available, fall back to signatures.
    Returns (ext, description) or None."""
...
def print_header():
    """Print header with detected capabilities."""
