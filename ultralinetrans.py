...
def is_english(text: str) -> bool:
    """Checks if text is English (ASCII)."""
    return not NON_ENGLISH_PATTERN.search(text)


def batch_translate(texts: list[str]) -> list[str]:
    """Translates a list of strings efficiently using a separator."""
    if not texts:
        return []
...
def translate_python_file(source: str) -> str:
    """Translates Python comments and strings while preserving code structure."""
    try:
