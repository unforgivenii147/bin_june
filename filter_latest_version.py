...
def parse_wheel_url(url: str) -> Optional[Tuple[str, str, Tuple[int, ...], str]]:
    """
    Parse wheel URL to extract package, python_version, version, and arch.

    Supports patterns:
    - maturin-1.13.3-py3-none-android_24_armeabi_v7a.whl
    - grpcio-1.59.2-cp310-cp310-linux_arm.whl
    - numpy-1.24.1-cp310-cp310-linux_arm.whl
    - polars-1.27.0-cp311-cp311-linux_armv7l.whl
    """
    # Pattern for android wheels
    android_pattern = r"/([^/]+)-(\d+\.\d+\.\d+)-py3-none-android_24_([^/]+)\.whl"
    # Pattern for linux wheels with cp tags
    ...
    if match:
        ...
        version = tuple(map(int, match.group(2).split(".")))
        arch = match.group(3)
        # Extract python version from URL path
...
def is_armv7_arch(arch: str) -> bool:
    """Check if architecture is ARMv7 compatible."""
...
def filter_latest_for_armv7(urls_file=None):
    """Filter latest version for ARMv7 architecture for each package and Python version."""
    ...
    # Group by (package, python_version) and track latest version
    ...
    # Output results grouped by package
    print("=" * 80)
