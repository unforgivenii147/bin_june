...
GZIP_COMPRESS_LEVEL = 9  # Maximum compression level (1-9, where 9 is best)
...
def decompress_file(path: Path) -> bool:
    ...
    out_path = path.with_suffix("")  # Remove .gz extension
...
def compress_chunk(data: bytes) -> bytes:
    """Compress a single chunk with max gzip settings."""
    ...
    try:
        ...
        with (
            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                ...
                # Note: This creates concatenated gzip streams
                # Each chunk is a valid gzip stream that can be decompressed independently
...
def compress_tar_to_gz(tar_path: Path, gz_path: Path) -> bool:
    """Compress a tar file to tar.gz format."""
    try:
...
async def compress_folder_async(folder_path: Path, output_base_name: str) -> bool:
    """Compress a folder to tar archive, then compress to tar.gz."""
...
async def process_decompress() -> None:
    ...
    # Process .tar.gz archives first (these are compressed folders)
    ...
    if archives:
        ...
        for archive in sorted(archives):
            ...
            tar_path = None
            try:
                # First decompress the .gz to .tar
                ...
                extract_dir = archive.stem  # Remove .tar.gz
...
Gzip Settings:
  ...
  - Note: Gzip is widely supported and fast for decompression
    ...
    group.add_argument(
        ...
        "--decompress",
        action="store_true",
        help="Decompress .gz and .tar.gz files",
    )
