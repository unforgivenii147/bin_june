            ...
            with tarfile.open(tmp_tar_path, 'w') as tar:
                ...
                for item in current_dir.rglob('*'):
                    if '.git' in item.parts or item.name.endswith('.tar.zst'):
                        continue
                    files.append(item)
                with ThreadPoolExecutor(max_workers=min(8, mp.cpu_count())
                    ) as executor:
                    list(executor.map(lambda f: tar.add(f, arcname=f.
                        relative_to(parent_dir)), files))
        ...
        with open(archive_path, 'wb') as f_out:
            with compressor.stream_writer(f_out) as zstd_writer:
                with tarfile.open(fileobj=zstd_writer, mode='w|') as tar:
                    files = []
                    for item in current_dir.rglob('*'):
                        if '.git' in item.parts or item.name.endswith(
                            '.tar.zst'):
                            continue
                        files.append(item)
                    for file_path in files:
                        tar.add(file_path, arcname=file_path.relative_to(
                            parent_dir))
        if archive_path.exists() and archive_path.stat().st_size > 0:
            shutil.rmtree(current_dir)
        else:
