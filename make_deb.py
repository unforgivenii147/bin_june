...
def should_exclude(pkg_name: str) -> bool:
    ...
    # Check patterns for llvm/clang related packages
    if any(exclude in pkg_lower for exclude in ["llvm", "clang"]):
        return True

    # Check patterns for rust/cargo related packages
...
def create_deb_for_package(pkg_name: str) -> bool:
    """Create .deb file for a single package."""
    try:
        # Create DEB_DIR if it doesn't exist
...
def process_packages(packages: List[str], max_workers: int = 4) -> tuple[int, int]:
    ...
    # Filter out excluded packages and remove duplicates
