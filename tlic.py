...
def is_text_file(filepath: Path) -> bool:
    """Check if file is likely a text file by looking for null bytes."""
    try:
...
def extract_blocks_from_file(filepath: Path, min_lines: int = 2) -> List[Tuple[str, int, List[str]]]:
    """
    Extract multi-line blocks from a file, skipping shebang lines.
    Returns list of (block_text, start_line_number, original_lines).
    """
    try:
    ...
    i = 0
    while i < len(lines):
        # Check if current line is shebang
        ...
        # Collect consecutive non-shebang lines
        while i < len(lines):
            stripped = lines[i].strip()
            if stripped.startswith("#!"):
                break
        ...
        # Only keep blocks that meet minimum line count
        if len(block_stripped) >= min_lines:
            block_text = "\n".join(block_stripped)
            blocks.append((block_text, block_start + 1, block_lines))  # +1 for 1-based line numbers
...
def collect_blocks_parallel(
    ...
    """Scan all text files and collect multi-line blocks in parallel."""
...
def find_repeated_blocks(
    ...
    """Filter to only keep blocks that appear multiple times (in one or multiple files)."""
...
def report(repeated: Dict[str, List[Tuple[Path, int, List[str]]]], root: Path) -> None:
    ...
    for i, (block_text, occurrences) in enumerate(repeated.items(), 1):
        ...
        # Count unique files vs total occurrences
...
def process_file_removal(filepath: Path, removals: List[Tuple[int, List[str]]], root: Path) -> Tuple[Path, int, bool]:
    """
    Remove specified blocks from a file.
    Returns (filepath, lines_removed, success).
    """
    try:
        with open(filepath, encoding="utf-8", errors="replace") as f:
    ...
    for start_lineno, block_lines in removals:
        for offset in range(len(block_lines)):
            lines_to_remove.add(start_lineno + offset - 1)  # Convert to 0-based
...
def remove_repeated_blocks(repeated: Dict[str, List[Tuple[Path, int, List[str]]]], root: Path, n_jobs: int = 8) -> None:
    """
    Remove all occurrences of repeated blocks from files.
