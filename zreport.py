    ...
    try:
        with open(filepath, "rb") as f:
            ...
            if frame_params.uncompressed_size:
                ...
                # Fallback: decompress and count bytes (slower but accurate)
                # This is a fallback for files without embedded uncompressed size
                f.seek(0)
                decompressed = dctx.stream_reader(f)
                size = 0
                while True:
                    chunk = decompressed.read(1024 * 1024)  # 1MB chunks
                    if not chunk:
                        break
                    size += len(chunk)
                return size
