    ...
    try:
        ...
        if dst.suffix == ".tar":
            try:
                ...
                dst.unlink()
                path.unlink()
                return {"status": "ok", "path": str(path), "extracted": True, "original": len(data), "decompressed": len(decompressed)}
            except Exception as e:
                return {"status": "error", "path": str(path), "error": f"tar extract: {e}"}
                
        path.unlink()
        return {"status": "ok", "path": str(path), "dst": str(dst), "original": len(data), "decompressed": len(decompressed)}
    except Exception as e:
        dst.unlink(missing_ok=True)
        return {"status": "error", "path": str(path), "error": str(e)}
        ...
        if files:
            with ProcessPoolExecutor(max_workers=args.workers) as pool:
                ...
                for fut in as_completed(futures):
                    res = fut.result()
                    if res["status"] == "ok":
                        logger.info(f"  ✓ {Path(res['path']).name}: {fsize(res['original'])} → {fsize(res['compressed'])}")
        else:
