...
def extract_metadata_from_wheel(wheel_path: Path) -> Optional[Dict[str, str]]:
    """
    Extract metadata from a .whl file by reading the METADATA file inside.
...
def extract_wheel_tags(filename: str) -> Optional[Tuple[str, str, str]]:
    """
    Extract Python tag, ABI tag, and platform tag from wheel filename.
    Returns (python_tag, abi_tag, platform_tag) or None.
    """
...
def reconstruct_wheel_name(wheel_path: Path, metadata: Dict[str, str], original_filename: str) -> Optional[str]:
    """
    Reconstruct the proper wheel name from metadata and original filename tags.
    """
    name = metadata["name"]
...
def fix_whl_files_by_metadata(directory: str = ".", dry_run: bool = True, backup: bool = True):
    ...
    Args:
        directory: Path to directory containing .whl files
        dry_run: If True, only show changes without renaming
        backup: Create backups before renaming
    """
    path = Path(directory)
