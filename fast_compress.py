...
def get_files(directory: Path, compress: bool):
    ...
    Sorts files by size (largest first) for efficient processing.
    """
    files = []
    total_dirs = 0
    total_files = 0
    ...
    # Sort files by size (largest first) for efficient processing
    print(f"Sorting {len(files)} files by size (largest first)...")
...
def main():
    ...
    # Validate arguments
    if args.compress and (args.level < 1 or args.level > 21):
