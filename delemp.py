...
def print_header(paths: list[Path], preserve_single: bool, remove_spaces: bool, mmap_threshold: int):
    ...
    print(f"  mmap threshold: {format_size(mmap_threshold)}")
    print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s                          # Remove all blank lines in current directory
  %(prog)s file.py                  # Process single file (remove all blank lines)
  %(prog)s -1 .                     # Preserve single blank lines
  %(prog)s -1 dir1 file.py          # Multiple targets, preserve single blank lines
  %(prog)s -s .                     # Remove blank and whitespace-only lines
  %(prog)s -1 -s dir1 file.py       # Preserve single blanks, remove whitespace-only
  %(prog)s -w 8 /path/to/dir        # Use 8 worker processes
  %(prog)s -t 2097152 .             # Set mmap threshold to 2MB
        """,
    )
    ...
    print(f"{DIM}(Using {max_workers} worker processes, mmap for files > {format_size(MMAP_THRESHOLD)}){RESET}\n")
