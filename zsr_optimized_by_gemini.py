...
CHUNK_SIZE: Final[int] = 512 * 1024  # 512 KiB
...
def compress_chunked(in_path: Path, out_path: Path, file_size: int) -> bool:
    """Compress a file in chunks using mmap and parallel processing."""
    try:
...
async def compress_folder_async(folder_path: Path, output_base_name: str) -> bool:
    """Asynchronously archive and compress a folder."""
...
async def process_compress() -> None:
    ...
    files = [p for p in cwd.iterdir() if p.is_file() and not p.suffix in (".zst", ".tar", ".gz", ".zip") and p.stat().st_size >= 1024]
    if files:
...
async def process_decompress() -> None:
    """Handle the decompression workflow."""
    cwd = Path.cwd()
