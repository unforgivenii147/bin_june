...
def get_files_generator(directory: Path, compress: bool):
    """
    Generator that recursively yields files to process using Path.walk().
    Skips .git directories, symlinks, and unwanted extensions.
    Uses a heap to sort files by size (largest first) while streaming.
    """
    total_dirs = 0
    total_files = 0
    skipped_symlinks = 0
    ...
    # Use a heap to collect files with their sizes for sorting
    # We'll use a negative size for max-heap behavior (largest first)
    file_heap = []
    heap_size_limit = 10000  # Process in batches to avoid memory issues
    ...
    for root, dirs, file_names in directory.walk():
        ...
        # Check if this is an editable package directory
        ...
        for file_name in file_names:
            ...
            # Get file size and add to heap (negative for max-heap)
            try:
            ...
            # Yield files in batches when heap gets large
    ...
    # Yield remaining files in sorted order
...
def process_files(
    ...
    """Process files from generator with progress display."""
    stats = SpaceStats()
    failed = []
    skipped = 0
    completed = 0
    ...
    # First, collect all files from generator to know total count for progress
    # This is necessary for progress bar, but we could also show progress without total
