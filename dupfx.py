...
def should_skip(path: Path) -> bool:
    return bool(
        path.is_symlink()
        or not (size := path.stat().st_size)  # Store size, skip empty files
...
def find_duplicates() -> None:
    ...
    for size, paths in files_by_size.items():
        if (count := len(paths)) > 1:  # Walrus: store count
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for future in as_completed(future_to_path):
            if hash_result := future.result()[0]:  # Walrus: check if hash exists
    ...
    if duplicate_count > 0:
        ...
        for hash_val, paths in files_by_hash.items():
            if (count := len(paths)) > 1:  # Walrus: store count
                for file_path in paths[1:]:  # Keep first, delete rest
                    relative_path = file_path.relative_to(cwd)
