...
# Pattern for various non-Latin scripts (Arabic, CJK, Cyrillic)
...
def is_foreign_line(line: str) -> bool:
    """Checks if a line contains non-Latin scripts."""
    return bool(LANGUAGE_PATTERN.search(line))


def process_file(file_path: Path) -> str:
    """Processes a single file, translating foreign lines in-place."""
    try:
            ...
            if stripped and is_foreign_line(stripped):
                try:
                    ...
                    if translated:
                        ...
                        # Preserve original ending if it was a full line
                        ending = "\n" if line.endswith("\n") else ""
...
def main() -> None:
    ...
    for ext in extensions:
        # Avoid processing hidden files and directories in SKIP_DIRS
