...
def scan_package(package_path: Path, site_dir: Path) -> Dict[str, any]:
    ...
    try:
        ...
        # Only determine pure status if package doesn't have entry_points
        if not has_entry_points:
...
def find_packages_without_entry_points(site_dir: Path) -> Tuple[List[str], List[str]]:
    ...
    Returns tuple (pure_python_packages, non_pure_packages) as package names only.
    """
    pure_packages = []
    non_pure_packages = []
...
def main():
    ...
    # Write pure Python packages (package names only)
    if args.json:
        pure_data = {
    ...
    # Write non-pure packages (package names only)
    if args.json:
        nonpure_data = {
            "timestamp": datetime.now().isoformat(),
            "site_directories": [str(d) for d in site_dirs],
