...
def find_dist_info_dirs(site_packages: Path) -> list[Path]:
    """Find all distribution info directories in site-packages."""
    dist_dirs = []
...
def get_package_name_version(dist_dir: Path) -> tuple[str, str]:
    """Extract package name and version from distribution directory name."""
    name = dist_dir.name
...
def read_record_file(dist_dir: Path, site_packages: Path) -> tuple[list[Path], set[Path]]:
    """Read RECORD file and return existing and missing files."""
...
def get_wheel_tag(dist_dir: Path) -> str | None:
    """Extract wheel tag from WHEEL file."""
...
def create_wheel(pkg_name: str, pkg_version: str, temp_dir: Path, output_dir: Path, wheel_tag: str | None) -> bool:
    """Create a wheel file from the temporary directory contents."""
    try:
...
def repack_package(dist_dir: Path, site_packages: Path, output_dir: Path, not_repacked_dir: Path) -> bool:
    """Repack a single package into a wheel."""
    ...
    if has_missing_critical:
        # Copy to not-repacked directory instead
...
def main() -> None:
    ...
    # Use current directory as site-packages (for Termux compatibility)
    site_packages = Path.cwd()

    # Standard site-packages detection (commented out for Termux)
    # site_packages = (
    #     Path(sys.prefix) / "lib" /
    ...
    # Filter by specified packages if not using --all
    if not args.all:
