...
BZ2_COMPRESS_LEVEL = 9  # Maximum compression level (1-9, where 9 is best)
...
def decompress_file(path: Path) -> bool:
    ...
    out_path = path.with_suffix("")  # Remove .bz2 extension
...
def compress_chunk(data: bytes) -> bytes:
    """Compress a single chunk with max bzip2 settings."""
...
def compress_tar_to_bz2(tar_path: Path, bz2_path: Path) -> bool:
    """Compress a tar file to tar.bz2 format."""
    try:
...
async def compress_folder_async(folder_path: Path, output_base_name: str) -> bool:
    """Compress a folder to tar archive, then compress to tar.bz2."""
    ...
    try:
        ...
        # Step 2: Compress tar to tar.bz2
...
async def process_decompress() -> None:
    ...
    # Process .tar.bz2 archives first (these are compressed folders)
    ...
    if archives:
        ...
        for archive in sorted(archives):
            ...
            tar_path = None
            try:
                # First decompress the .bz2 to .tar
                ...
                extract_dir = archive.stem  # Remove .tar.bz2
