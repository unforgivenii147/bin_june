...
def has_python_files(dir_path: Path) -> bool:
    """Check if directory contains any Python files (recursively, skipping SKIP_DIRS)."""
    try:
...
def find_imports_for_directory(dir_path: Path, start_path: Path, std_libs: set, all_local_packages: set) -> list:
    """Find third-party imports for a specific directory. Returns list of module names."""
    # Get all Python files in this directory and its subdirectories
    files = []
    ...
    # Get local modules for this specific directory
...
def save_requirements_file(modules: list, output_path: Path, pkgz: set) -> bool:
    """Save requirements.txt for given modules. Returns True if file was created."""
    ...
    # Remove installed packages, private modules, and duplicates
    seen = set()
    unique_cleaned = []
    for p in cleaned:
    ...
    # If nothing left after cleaning, remove the file
    if not unique_cleaned:
        if output_path.exists():
            output_path.unlink()
        return False

    # Write the final cleaned unique content
...
def get_valid_subdirs(start_path: Path) -> list:
    """Get list of valid subdirectories (not in SKIP_DIRS and contain Python files)."""
    subdirs = []
...
def main() -> None:
    ...
    # Get all local packages from the entire tree
    ...
    if args.save_separate and subdirs:
        ...
        # Process each subdirectory and generate its own requirements.txt
        total_imports = set()
        created_count = 0
        skipped_count = 0
        ...
        for idx, subdir in enumerate(subdirs, 1):
            ...
            if not modules:
                # No third-party modules found at all (all were local/stdlib)
                elapsed = time.time() - dir_start
                print(f"⏭️  no third-party imports ({elapsed:.2f}s)")
                skipped_count += 1
                continue

            # Try to save requirements file
        ...
        # Now generate the root requirements.txt with all unique combined imports
        if total_imports:
            ...
            # Make sure we don't have a stale requirements.txt
            if output_file.exists():
                output_file.unlink()

    else:
        # Standard mode - process entire directory as before
        print("\n📦 Processing entire directory...")

        # Get all Python files, skipping SKIP_DIRS
        files = []
        ...
        if modules:
            ...
            # Remove any existing requirements.txt if no imports found
            if output_file.exists():
                output_file.unlink()
