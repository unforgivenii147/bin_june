...
def main() -> None:
    ...
    parser.add_argument(
        "--workers", "-w", type=int, default=mp.cpu_count(),
        ...
        if path.is_file():
            ...
            for ext in args.extensions:
                for fp in path.rglob(f"*{ext}"):
                    if (fp.is_file() and 
                        fp.resolve() not in exclude_paths and
                        not any(part.startswith(".") for part in fp.parts)):
    ...
    logger.info("Found %d files. Using %d workers (Threshold: %.0f%%)", 
                len(files_to_process), args.workers, args.threshold * 100)
