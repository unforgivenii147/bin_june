...
DEFAULT_SKIP_DIRS: Final[set[str]] = {
    "zstandard", "0", "compressed", "faprint", "packaging", "joblib",
    "loguru", "setuptools", "pip", "wheel", ".git", "dist", "build",
...
def compress_folder_task(
    ...
    try:
        ...
        if zst_path.exists(): zst_path.unlink()
...
def main():
    ...
    if successes:
        ...
        if args.decompress:
            ...
            print(f"Space saved: {format_size(total_orig - total_proc if 'total_proc' in locals() else total_orig - total_comp)}")
    print(f"{'='*40}")
