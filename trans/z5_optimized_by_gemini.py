...
SKIP_DIRS: Final[frozenset[str]] = frozenset({
    "lazy", ".git", ".svn", ".hg", "__pycache__", 
    ".mypy_cache", ".ruff_cache", ".pytest_cache",
    "node_modules", ".venv", "venv", ".env"
})

EXCLUDED_EXTENSIONS: Final[set[str]] = {
    ".xz", ".zst", ".zstd", ".7z", ".gz", ".bz2", ".zip", ".rar", ".tar", ".tgz",
    ".tbz2", ".txz", ".tlz", ".lz", ".lz4", ".lzma", ".lzo", ".sz", ".snappy",
    ".zlib", ".deflate", ".flac", ".mp3", ".aac", ".ogg", ".wma", ".opus", ".m4a",
    ".wavpack", ".jpg", ".jpeg", ".png", ".gif", ".webp", ".avif", ".heic", ".heif",
    ".mp4", ".avi", ".mkv", ".mov", ".wmv", ".flv", ".webm", ".m4v", ".pdf",
    ".docx", ".xlsx", ".pptx", ".odt", ".ods", ".odp", ".exe", ".dll", ".so",
    ".dylib", ".wasm", ".whl", ".egg", ".deb", ".rpm", ".apk", ".ipa", ".pyc",
    ".pyo", ".class", ".o", ".obj", ".lib", ".a", ".iso", ".img", ".dmg", ".vdi",
    ".vmdk", ".qcow2"
}
...
class OperationResult:
    """Stores the outcome of a compression or decompression operation."""
    path: Path
    original_size: int
    processed_size: int
    success: bool
...
def compress_file(
    ...
    """Compress a single file using streaming Zstandard."""
...
def decompress_file(
    ...
    """Decompress a single file using streaming Zstandard."""
    start_time = time.perf_counter()
    try:
        orig_size = input_path.stat().st_size
...
def get_files(
    ...
    """Gather files to process based on criteria."""
    found = []
...
def print_summary(results: list[OperationResult], root: Path, operation: str):
    ...
    if RICH_AVAILABLE:
        ...
        summary = Text.assemble(
            ...
            (f"Ratio: ", "dim"), (f"{((1 - total_proc/total_orig)*100 if total_orig > 0 else 0):.1f}%\n", "bold green"),
...
def main():
    ...
    parser.add_argument("-l", "--level", type=int, default=9, help="Compression level (1-22)")
    ...
    # Gather files
    files = get_files(
        root, mode, EXCLUDED_EXTENSIONS, args.exclude, 
        ext_filter=args.ext, recursive=not args.tar
    )
